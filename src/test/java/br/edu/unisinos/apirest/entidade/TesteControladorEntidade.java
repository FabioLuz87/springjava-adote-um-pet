...
class TesteControladorEntidade {
    ...
    @Autowired RepositorioEntidade repository;
    ...
    @Test void shouldCompleteCrud() throws Exception {
        ...
        String location = mvc.perform(post("/api/v1/entidades").contentType(MediaType.APPLICATION_JSON).content(body))
                .andExpect(status().isCreated()).andExpect(jsonPath("$.nome").value("Abrigo Amigo"))
                .andReturn().getResponse().getHeader("Location");
        mvc.perform(get(location)).andExpect(status().isOk()).andExpect(jsonPath("$.endereco.estado").value("RS"));
