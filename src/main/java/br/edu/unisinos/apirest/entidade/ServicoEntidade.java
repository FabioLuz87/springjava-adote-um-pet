...
public class ServicoEntidade {
    ...
    public ServicoEntidade(RepositorioEntidade repository) { this.repository = repository; }
    ...
    @Transactional(readOnly = true)
    public RespostaEntidade findById(Long id) { return RespostaEntidade.from(get(id)); }
    @Transactional
    public RespostaEntidade create(RequisicaoEntidade request) {
        if (repository.existsByEmailIgnoreCase(request.email())) throw new ExcecaoConflito("E-mail já cadastrado.");
    ...
    public RespostaEntidade update(Long id, RequisicaoEntidade request) {
        ...
        if (repository.existsByEmailIgnoreCaseAndIdNot(request.email(), id)) throw new ExcecaoConflito("E-mail já cadastrado.");
    ...
    public Entidade get(Long id) {
        return repository.findById(id).orElseThrow(() -> new ExcecaoRecursoNaoEncontrado("Entidade não encontrada."));
    }
}
