...
public class Usuario {
    ...
    @Column(nullable = false, length = 255) private String hashSenha;
    @Column(nullable = false, length = 30) private String perfil;
    @OneToOne(fetch = FetchType.LAZY, cascade = CascadeType.ALL, orphanRemoval = true, optional = false)
    @JoinColumn(name = "id_endereco", nullable = false, unique = true) private Endereco endereco;

    protected Usuario() {}
    public Usuario(RequisicaoUsuario request, String hashSenha) {
        endereco = new Endereco(request.endereco()); update(request, hashSenha);
    }
    public void update(RequisicaoUsuario request, String newPasswordHash) {
        nome = request.nome(); cpf = request.cpf(); email = request.email().toLowerCase(); telefone = request.telefone();
        perfil = request.perfil(); endereco.update(request.endereco());
        if (newPasswordHash != null) hashSenha = newPasswordHash;
    }
    ...
    public String getPerfil() { return perfil; } public Endereco getEndereco() { return endereco; }
}
