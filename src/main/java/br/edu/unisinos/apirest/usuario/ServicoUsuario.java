...
public class ServicoUsuario {
    ...
    public Page<RespostaUsuario> findAll(Pageable pageable) {
        return repository.findAll(pageable).map(RespostaUsuario::from);
    }

    @Transactional(readOnly = true)
    public RespostaUsuario findById(Long id) {
        return RespostaUsuario.from(get(id));
    }

    @Transactional
    public RespostaUsuario create(RequisicaoUsuario request) {
